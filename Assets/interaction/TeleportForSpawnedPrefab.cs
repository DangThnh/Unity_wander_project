...
public class TeleportForSpawnedPrefab : TeleportInteractionBase
{
    ...
    private IEnumerator WaitForGameManagerUIRegistration()
    {
        ...
        if (faderCanvasGroup == null)
        {
            ...
            {
                // Thông báo nếu không tìm thấy Fader
    ...
    // Ghi đè Update để xử lý Input Panel và vô hiệu hóa các phím chức năng khác
    private new void Update()
    {
        // Chỉ xử lý Input ở trạng thái Question Panel
        if (isInteracting)
        {
            // VÔ HIỆU HÓA PHÍM E VÀ C KHI ĐANG TƯƠNG TÁC CÂU HỎI
            if (Input.GetKeyDown(KeyCode.E) || Input.GetKeyDown(KeyCode.C))
            {
                return; // Thoát hàm ngay lập tức, không cho phép thực hiện hành động của các phím này
            }
    ...
    private void ShowQuestionUI()
    {
        ...
        // Đã loại bỏ lệnh khóa chuyển động theo yêu cầu cũ
    }
    ...
    private new IEnumerator FadeAndTeleport()
    {
        ...
        if (faderCanvasGroup == null)
        {
            ...
            // Dịch chuyển nhân vật qua GameManager
